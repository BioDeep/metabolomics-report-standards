...
The work of this metabolomics identification report standards from the BioNovoGene R&amp;D labs is based on the research works of: 
...
> 3. ``pvalue`` for the metabolites' identification is tested from the ``BHR`` score vector, and then test by the in-house hyper-geometric algorithm which is developed by BioNovoGene. The ``BHR`` vector which is original defined in this literature article: "KAAS: an automatic genome annotation and pathway reconstruction server." **_DOI: 10.1093/nar/gkm321_**. 
...
> 7. ``identify.level`` confidence levels is defined by BioNovoGene, this ``identify.level`` concepts is developed from this literature article: "Identifying Small Molecules via High Resolution Mass Spectrometry: Communicating Confidence". **_DOI: 10.1021/es5002105_**
